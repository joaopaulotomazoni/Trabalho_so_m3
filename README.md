...
struct File {
    ...
    int    owner_id;   // ID do usuário proprietário
...
| `permissoes`       | Modo de acesso      | Bitmask rwx para owner e others                |
...
```
Octal: 0 6 4 4
        │ │ │ └── others: 100 = r-- (somente leitura)
        │ │ └──── group:  100 = r-- (somente leitura)
        │ └────── owner:  110 = rw- (leitura e escrita)
        └──────── sticky/setuid (não usado)
```
...
bool hasPermission(File* arquivo, int usuarioId, char operacao) {
    if (usuarioId == 0) return true;    // root: acesso total
...
| `su`    | `su <id>` | Muda o usuário atual (ex: `su 0` = root) |
